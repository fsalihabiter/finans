...
| T6.1 | **İlk müfredat içeriği (5 ders):** enflasyon & reel getiri · risk-getiri · çeşitlendirme/yoğunlaşma · maliyet ortalaması · BES'i doğru kullanmak. Her derste "Senin portföyünde" bağlam bloğu şablonu + 2-3 soruluk quiz. İçerik repo'da (topluluk katkısına açık, `14` §4-D2). **Genişletildi (`15` §2):** gövdeler **L1 Core / L2 Context / L3 Deep** katmanlı yazılır + jenerik örnek + tuzak bloğu | T5E.2 | `14` §4-A1, `15` §2 | [x] (2026-07-19; SC-E12; **25 bölüm** (5 ders × L1+L2+L3+örnek+tuzak) yeni `EducationContent.cs`'te — SeedData'dan ayrı tutuldu ki topluluk katkısına açılabilsin (`14` §4-D2); 2-5. derslere **3'er soruluk test** eklendi (quiz 1→5, soru 3→15). ⚠ **Ayrı idempotent kapı**: `SeedEducationAsync` "track var mı?" ile korunduğundan çalışan DB'ler içeriği ondan alamazdı → `SeedEducationSectionsAsync`/`SeedRemainingQuizzesAsync` kendi kapılarıyla **geriye dönük yükler**. İçerik MiniMarkdown alt kümesinde (tablo/link YOK → T6.8), tavsiye/tahmin ifadesi testle yasaklandı. **`LiveContext` bloğu bilinçli olarak T6.2'ye bırakıldı** (ContextKey sözleşmesi gerekiyor). Education 32/32; **canlı Postgres teyitli**) |
| T6.2 | **"Senin portföyünde" bağlam API'si:** ders başına portföyden türetilen bağlam değerleri (örn. çeşitlendirme dersi → kullanıcının yoğunlaşma oranı) — hesap KODDA, deterministik; LLM yok. **Genişletildi (`15` §3):** `LessonContextService` + `ContextKey` sözleşmesi (8 metrik, hepsi mevcut `PortfolioAnonymizer`/`PortfolioService` çıktısından) + **3 durum** (`Own`/`Demo`/`Stale`) | T5E.3, T1.7 | `14` §4-A1, `15` §3, `CLAUDE.md` §3.1 | [x] (2026-07-19; SC-E13; `ILessonContextService` + `ContextKeys` (9 anahtar) — şablon `{{token}}` yaklaşımı: ayrı kayıt tablosu yok, içerik yazarı token yazar; **Application katmanında** (DbContext'siz saf mantık → birim testli). 5 derse `LiveContext` bloğu eklendi (blok 5→6). Çözülemeyen token'ın **satırı düşer**. ⚠ Canlıda yakalanan iki hata: (a) üretim imajı *globalization-invariant* → `GetCultureInfo("tr-TR")` 500 verdi, açık `NumberFormatInfo`'ya geçildi + kültür-bağımsızlık testi; (b) seed kapısı blok bazına çevrildi (deterministik Id) yoksa yeni blok tipi mevcut DB'lere inmiyordu. **İlerleme akışı** dahil: `NextLessonDto` + "Sonraki ders" geçişi + tamamlama bildirimi. Application 277/277, web 104/104; **canlı Postgres teyitli**) |
...
| T6.6 | **Tanılama testi (8 soru):** 4 bilgi (nesnel → `LiteracyLevel`) + 4 senaryo (puansız → `RiskAttitude`); atlanabilir, utandırmayan; `Users.LiteracyLevel/RiskAttitude/ProfiledAtUtc`. ⚠ **`RiskAttitude` arayüze/API yanıtına ÇIKMAZ** (SC-E4) — **eski T7.1'i devralır** | T6.5 | `15` §4, `14` §4-A2 | [x] (2026-07-19; SC-E14; `DiagnosticContent` soru bankası + puanlama (saf/deterministik) + `IDiagnosticService` + 3 uç (`GET diagnostic`·`GET profile`·`POST diagnostic`) + migration `UserLiteracyProfile`; web onboarding (profil ölçülmemişse dersten önce, "Atla" seçenekli). 🔒 Risk tutumu DB'de saklanır, **hiçbir DTO/yanıt/arayüzde yok** — ham gövde taramasıyla test edildi. Cevaplanmayan bilgi sorusu yanlış sayılır (atlamak derin içerik açmaz). App 291/291, web 107/107) |
| T6.7 | **Uyarlanabilir render (web):** seviyeye göre L1/L2/L3 katmanı + **"Daha derine in"** (tavan kapatılmaz — alt seviye kullanıcı isteğe bağlı derinleşebilir) | T6.5, T6.6 | `15` §2.2, `13` | [x] (2026-07-19; SC-E16; `<details>` ile katlama — seviyenin ÜSTÜNDEKİ katman katlanır, herkes açabilir; profil yoksa Başlangıç varsayılır. **Görselleştirme:** `LessonSection.FigureKey` + `LessonFigure` (5 elle yazılmış SVG, kütüphanesiz, tema uyumlu, `role="img"`); bilinmeyen anahtar sessizce atlanır. **Tuzak bloğu Core katmana alındı** (başlangıçtan gizlenmemeli). ⚠ Seed artık **mutabakat** yapıyor (metin/katman/figür değişikliği çalışan DB'ye iner) — salt-ekle yaklaşımı bunu kaçırıyordu. ⚠ Tarayıcı kontrolünde **MiniMarkdown hatası** bulundu: ardışık `> ` satırları 4 ayrı blockquote üretiyordu → birleştirildi. App 291/291, web 114/114; canlıda ekran görüntüsüyle teyit) |
| T6.8 | **`MiniMarkdown` genişletme:** tablo + link desteği (derin içerik için gerekli) — **`dangerouslySetInnerHTML` YİNE YOK** (XSS-güvenli, topluluk içeriği `14` §4-D2). 🔺 **ÖNE ALINDI (2026-07-22):** `Source` bloğundaki kurumsal referansların tıklanabilir olması için önkoşul (`16` §6.1) | — | `15` §2, `16` §6.1, `13` | [x] (2026-07-22; SC-E24; **şema beyaz-listesi** `safeHref` — `https/http/mailto` + uygulama içi `/yol`; güvensiz hedef (`javascript:`/`data:`/`vbscript:`, protokole göreli `//host`, **kontrol karakteriyle gizlenmiş şema**) bağlantıya çevrilmez, ham metin kalır. Dış bağlantı `target=_blank` + `rel="noopener noreferrer"` + `aria-hidden` ok. Tablo **yalnız hizalama satırı varsa** tablodur (yoksa düz paragraf — geriye dönük), sütun sayısı **başlıktan** gelir (eksik hücre boş, fazlası düşer), `:-:`/`---:` hizalaması okunur, **kendi kabında** yatay kaydırılır (sayfa taşmaz). ⚠ Kaynak dosyada kontrol karakteri regex'i kazaya yol açtı → karakter KODU ile süzülüyor (`stripHiddenChars`). Seed içerik kısıtı güncellendi: tablo/link serbest ama şema denetimli + hizalama satırı zorunlu. web **130/130**, Integration 15/15 (Education). ⚠ **Tarayıcı doğrulaması yapılamadı** — Chrome sekmesi localhost'u açamadı; görsel teyit T6.19'da gerçek kaynak bloklarıyla yapılacak) |
...
| T6.10 | *(kapsam genişledi)* **Aşamalı ders akışı:** bölümler adım adım okunur (tek adım/ekran) + **yol haritası** (ilerideki başlıklar GÖRÜNÜR ama kilitli — merak + hedef) + adım göstergesi; **mini test AYRI ve SON adım** (sayfanın devamı değil, yönlendirmeyle ulaşılır) | T6.7 | `15` §2.2, `13` §4 | [x] (2026-07-19; SC-E17; `buildSteps`/`LessonRoadmap`/`StepContent`; bölüm başlığı gövdeden **ayrıştırılıp** `Heading`'e taşındı (çift gösterim yok, yol haritası okunur); seviyenin üstündeki adım "ileri seviye — istersen atla" ile işaretlenir; bölümsüz derste tek parça okuma korunur. ⚠ Seed mutabakatı `Heading`'i atlıyordu → alan listesi eksiksizleştirildi + test. web 116/116; canlıda ekran görüntüsüyle teyit) |
...
| **T6.15** | **Çok set desteği (web):** eğitim sayfası `tracks.data[0]` varsayımından kurtulur → set listesi + set başına ilerleme + **"Buradan başla" rozeti** (`LiteracyLevel`'a göre öneri; **kilit YOK**, §6.3) | T5E.4 | `15` §6.3, `13` | [x] (2026-07-26; SC-E19/E20. **Backend:** `LearningTrackDto`'ya `OrderIndex` + `CompletedCount` (kullanıcıya kapsanan set başına ilerleme, IN sorgusuyla tek çağrı); shared tip güncellendi. **Web:** `tracks.data[0]` kalktı → **tek set doğrudan açılır** (tek öğeli menü yok), **≥2 set → set seçici** (`TrackList`): her kart seviye çipi + ilerleme çubuğu + "Buradan başla" rozeti (Başlangıç/ölçülmemiş→en düşük OrderIndex, Gelişen+→sonraki), **hiçbir set kilitli değil** (§6.3); seçiciden derslere, "← Setlere dön" ile geri. EducationApi 19/19 (+`Track_completed_count_reflects...` izolasyon dahil), web **135/135** (4 yeni test), web build temiz, Application 291/291) |
| **T6.16** | **Set 0 iskeleti:** "İlk Adımlar" track'i (OrderIndex 0) + **10 ders** + track içi ön-koşul zinciri; **"Temeller" → "Yatırım Kavramları"** yeniden adlandırma (seed mutabakatıyla canlı DB'ye iner) | T6.15 | `15` §6 | [~] (2026-07-26; **⚖ ürün sahibi kararı: "tam zenginlik şimdi"** → T6.16+T6.17 birleşti, Set-0 dersleri **ders-ders tam derinlikte** iniyor. **Foundation tamam:** "Temeller"→**"Yatırım Kavramları"** (slug korundu, idempotent mutabakat), **`ilk-adimlar` track'i (OrderIndex 0)** + kendi idempotent kapısı (`SeedSet0Async`, canlı DB'ye iner). **S0-L1 "Yatırım nedir" tam:** 12 aşama (intro+3 katman+2 örnek+2 tuzak+bağlam+kaynak), **8 figür** (çok panelli anlatı: üç eylem·10.000₺ üç yol·değer↔sıfır-toplam·getiri kaynağı…), **9 soru/3 zorluk**, `investment` kavramı. Yapısal sözleşme testleri güncellendi (sayımlar 54→66/5→6/27→36; figür tekilliği **ders-içi** (§8.4); anlatım aşamasında figür serbest (§6.1); **M3 Set-0 ≥6 figür guard'ı aktive**). EducationSeed+Api **40/40**, web 135/135, build temiz. **İlerleme: S0-L1…L5 tam derinlikte indi (5/10).** S0-L5 "Varlık türleri turu" (2026-08-13): 15 bölüm, **9 figür** (3 çok panelli: same-money-three-forms·fund-wrapper·ownership-lending-axis), 9 soru/3 zorluk, 3 kavram (asset-class·ownership-vs-lending·liquidity), ön-koşul S0-L4→L5. ⚠ **Yasal:** sınıflar sıralanmaz/karşılaştırılmaz, likidite ekseni bir kalite sırası **değil** (metinde açık); yalnız sınıf adları, enstrüman/şirket adı yok. LiveContext `asset_class_weights` yerine mevcut `{{holding_count}}`+`{{asset_class_count}}` (yeni plumbing gerekmesin, S0-L4 emsali). EducationSeed+Api **41/41**, web 141/141, Application 291/291, build temiz; canlı Postgres'e indi (119 bölüm/10 ders). S0-L6 "Getiri nereden gelir?" (2026-08-13): 13 bölüm, **9 figür** (4 çok panelli: two-return-sources·total-return-sum·dividend-not-free·equity-vs-debt-return), 9 soru/3 zorluk, 3 kavram (return-source·capital-gain·cash-flow-return), ön-koşul S0-L5→L6. Getirinin İKİ kaynağı (değer artışı + nakit akışı), toplam getiri=%A+%B, temettü bedava değil (fiyattan düşer), kâğıt üstü↔gerçekleşmiş. ⚠ **Yasal:** şirket adı/sembol yok (soyut "A"), temettü strateji önerisine dönüşmez; LiveContext `price_change_12m` yerine mevcut `{{return_ratio}}` (yeni plumbing yok). Canlı Postgres: 132 bölüm/11 ders. **İlerleme: S0-L1…L6 tam (6/10). Kalan: S0-L7…L10.** ⚠ Canlı tarayıcı görsel polish teyidi (S0-L4 slider + S0-L5/L6 figürleri) Chrome eklentisi bağlanınca yapılacak |
...
| **T6.18** | **Enflasyon kaydırıcısı** (Set 0 Ders 4 · set başına 1 etkileşimli araç): saf/deterministik **istemci** hesabı, klavye erişilebilir, **tahmin/enstrüman dili yok**, bileşen düşerse statik figür fallback | T6.17d | `15` §6.2, `16` S0-L4 | [x] (2026-07-26; SC-E23; `InflationSlider` (`figureKey="inflation-slider"`) → iki native range (klavye erişimli, `accent-color`+focus ring), hesap istemcide saf: `100/(1+i)^n` bileşik erime, TR biçim. **Tahmin ÜRETMEZ:** "şu oran **olursa**" (figcaption + readout), "olacak" yok; **enstrüman adı yok** (15 §3.4). SVG `aria-label` deterministik (aynı girdi → aynı çıktı). Bilinmeyen anahtar → `null` (LessonFigure) = fallback; ders 8 statik figürüyle eksiksiz. 4 bileşen testi (`LessonFigure.test.tsx`): 2 klavye slider · deterministik %0→100 · olursa/olacak + varlık adı taraması · fallback boş DOM. S0-L4'ün 6. aşaması) |
...
| **T6.21** | 🔴 **Enflasyon verisi gerçek değil:** `SeedData` `InflationRate`'i **`Source = "TÜİK"` etiketiyle sabit %38** yazıyor (kod içi yorum "placeholder" diyor). Eğitim artık **"kaynak daima görünür"** iddiası taşıyor (`16` §6) — kullanıcıya TÜİK etiketli **uydurma bir oran** göstermek bu iddiayı çürütür. Ya gerçek TÜFE çekilmeli ya da veri **açıkça "örnek"** etiketlenip reel getiri/`inflation_12m` çıktılarında böyle gösterilmeli | T6.19 | `16` §6.4, `14` §B1 | [x] (2026-07-26; canlı TÜFE beslemesi olmadığından **"örnek" etiketi** yolu seçildi: `InflationRate.Source` varsayılanı ve seed satırı `"TÜİK"` → **`"örnek"`** (domain doc + yorum güncellendi); S1-L1 "Senin portföyünde" bloğuna **reel getirinin örnek enflasyona göre hesaplandığı** dürüst uyarısı eklendi (nominal=gerçek, reel=örnek). Regresyon testi: seed'lenen oran kurumsal kaynakla (TÜİK/TCMB/SPK/BDDK) etiketlenemez. ⚠ `Source` alanı bugün hiçbir API/DTO'da kullanıcıya dönmüyordu ama stored yanlış iddia + türetilen `real_return`'ün sahte girdiden gelmesi giderildi. Gerçek TÜFE çekme ayrı iş olarak açık kalır. Integration (EducationSeed) 21/21) |
| **T6.20** | **Yapısal sözleşme testleri:** `16` §9.1'deki M1-M9 kuralları `EducationSeedTests`'e (Source bloğu · açılış bloğu · set bazlı figür eşiği · **figür anahtarı ↔ `LessonFigure.tsx` kayıt defteri mutabakatı** · 9 soru/3 zorluk · kavram kapsaması · tavsiye taraması). ⚠ **İçerikten SONRA** yazılır (T6.19), kırmızı test bırakılmaz | T6.19 | `16` §9 | [x] (2026-07-26; 5 yeni test. **M4** figür anahtarı ↔ `LessonFigure.tsx` kayıt defteri mutabakatı (TSX regex'le okunur, repo kökü bin'den yukarı yürünerek bulunur; seed'deki 11 anahtar ∈ 12'lik kayıt defteri) · **M6** boşta kavram yok + her ders ≥1 kavram · **M7** tavsiye/tahmin/**enstrüman-sıralama** taraması `Source` blokları dahil · **M3/M5 değişmezleri** (9+ soruluk quiz her zorluktan ≥3; Ders 1-2 zenginlik regresyon guard'ı). ⚠ **M3/M5 SAYISAL set eşikleri** (Set 0 ≥6, Set 2 ≥8, diğer ≥2; 9 soru/ders) bugün **global uygulanamaz** — Ders 3-5 hâlâ 1 figür + 3 soru (T6.11c bekliyor); §9.3 gereği kırmızı bırakılmadı, eşikler her setin içeriği indikçe (T6.11c/T6.16/T6.22) açılacak. M1/M2 T6.19'da, M8/M9 mevcut testlerde zaten yeşildi. EducationSeed 21/21, Application 291/291) |
