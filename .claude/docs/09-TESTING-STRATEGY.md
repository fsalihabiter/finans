...
| SC-E18 | 15 §6, `CLAUDE.md` §2 | **Zenginleştirilmiş ders + zorluk kademeli test** (T6.11a): ders başına blok sayısı SERBEST (Ders 1: 13); her testte **en az bir Easy** soru (yoksa Başlangıç hiç soru görmez, kapı hiç açılmaz); çoktan seçmelide birden çok doğru, tek seçimde tam bir doğru; **soru seviyeye göre filtrelenir ve AYNI küme üzerinden puanlanır** (görülmeyen sorudan kalınmaz); `Difficulty` migration'ı `defaultValue:'Easy'` (CHECK ihlali önlendi); **çok satırlı liste öğesi tek maddede birleşir** (regresyon) | Integration (3) + Unit (1) | [x] (2026-07-19; App 291/291, Integration 167/171, web 117/117; canlı tarayıcı kontrolü) |
| SC-E17 | 15 §2.2, `13` §4 | **Aşamalı ders akışı** (T6.10): ders **tek adım/ekran** gösterir (hepsi birden dökülmez); **yol haritası** ilerideki adımların BAŞLIĞINI gösterir ama düğmeleri **kilitlidir**; tamamlanan adıma geri dönülebilir; devam düğmesi **sonraki adımın adını** söyler; **mini test ayrı ve son adım** — okuma adımlarında test içeriği hiç görünmez, "Mini teste geç" ile ulaşılır; seviyenin üstündeki adım "ileri seviye" işaretli (İleri seviyede işaret yok); figür kendi adımında çizilir; bölümsüz derste yol haritası yok (tek parça); **her bölüm ders içinde tekil, gövdesiz-başlık** (seed) | Bileşen (9) + Integration (1) | [x] (2026-07-19; web 116/116, Integration 167/171; canlı tarayıcı kontrolü) |
| SC-E16 | 15 §2.2, `13` §4 | **Uyarlanabilir render + görselleştirme** (T6.7): kullanıcının seviyesinin ÜSTÜNDEKİ derinlik katmanı `<details>` içinde katlanır, seviyesindekiler açık; **katlanan içerik kimseden gizlenmez** (açılabilir); İleri seviyede hiçbir katman katlanmaz; profil yoksa Başlangıç varsayılır; figür anahtarı olan bölümde erişilebilir SVG (`role="img"`) çizilir, **bilinmeyen anahtar içeriği bozmaz**; seed **mutabakatı** düzenlenen içeriği (metin/katman/figür) çalışan DB'ye indirir; **ardışık `> ` satırları TEK blockquote** (regresyon) | Bileşen (5) + Integration (2) + Unit (2) | [x] (2026-07-19; web 114/114, Integration 166/170; canlı tarayıcı kontrolü) |
| SC-E15 | 15 §4, `CLAUDE.md` §2 | **Öğrenme kapısı + sıfır başlangıç** (karar 2026-07-19): seed **hiç ilerleme yazmaz** (herkes sıfırdan; Ders 1 açık, 2-5 türetilmiş kilitli); testi olan ders **test geçilmeden tamamlanamaz** → `PUT progress {Completed}` **400 `quiz_not_passed`** (kural SUNUCUDA); testi **geçmek dersi tamamlar** ve yalnız bir sonrakini açar; **kalmak** tamamlamaz; `InProgress` kapıya takılmaz | Integration (5) | [x] (2026-07-19; Education 37/37; **canlıda uçtan uca**: sıfır liste → 400 → test geç → 1 Completed, 2 açık, 3 kilitli) |
| SC-E14 | 15 §4, §1.1, `11` §3-4 | **Tanılama testi** (T6.6): 8 soru (4 bilgi + 4 senaryo) **cevap anahtarı sızmadan** servis edilir; doğru sayısı → `LiteracyLevel` (0-1 Başlangıç · 2-3 Gelişen · 4 İleri), **cevaplanmayan yanlış sayılır**; senaryo puanı → `RiskAttitude` (atlayan **Dengeli**); **🔒 `RiskAttitude` DB'ye yazılır ama HİÇBİR yanıtta/arayüzde görünmez** (SC-E4); yönlendirme mesajı puan/yanlış **anmaz** ve tutum etiketi geçmez; bilinmeyen soru/şık **400**; profil **per-user izole**; test **atlanabilir** | Unit (10) + Integration (6) + Bileşen (3) | [x] (2026-07-19; Application 291/291, web 107/107; sızıntı taraması ham gövde üzerinde) |
| SC-E13 | 15 §3, `CLAUDE.md` §3.1/§8 | **"Senin portföyünde" bağlamı** (T6.2): `{{anahtar}}` token'ları KODDA hesaplanmış metriklerle çözülür (LLM yok); **3 durum** — `Own` (gerçek veri) · `Demo` (portföy yok/<2 kalem/servis çöktü → etiketli örnek, NFR-5) · `Stale` (fiyat >24s eski, `AsOf` damgalı); **çözülemeyen token'ın satırı düşer** (ham `{{...}}` gösterilmez, uydurma "%0" yazılmaz); tür ağırlığı o türün TÜM dilimlerini toplar; TR biçim (binlik nokta/ondalık virgül) **ortam kültüründen bağımsız**; aynı portföy → aynı çıktı (SC-E8); ders **ilerleme akışı**: tamamlanınca sonraki ders açılır ve doğrudan geçilir, set sonunda geçiş sunulmaz | Unit (10) + Bileşen (3) | [x] (2026-07-19; Application 277/277, web 104/104; **canlı teyitli** — gerçek portföyde `Own`, hissesiz derste token satırı düştü) |
...
| SC-E19 | 15 §6.3, `13` | **Çok set:** eğitim sayfası birden fazla track listeler (ilk track'e sabitlenmiş varsayım kalmaz); her set kendi ilerlemesini gösterir; **hiçbir set kilitli değil** | Bileşen | [x] (2026-07-26 T6.15; `tracks.data[0]` kalktı → ≥2 set `TrackList` seçici, set başına `completedCount`/`lessonCount` ilerleme, önerilmeyen set de tıklanabilir (kilit yok); tek set doğrudan açılır. web 135/135) |
| SC-E20 | 15 §6.3, §4 | **"Buradan başla" önerisi:** `LiteracyLevel` Başlangıç/ölçülmemiş → Set 0 önerilir, Gelişen+ → Set 1; öneri bir **rozettir**, erişimi engellemez (tavan kapatılmaz) | Bileşen | [x] (2026-07-26 T6.15; `recommendedTrackId` — Başlangıç/null→en düşük OrderIndex, Gelişen+→sonraki set; tek rozet, tıklamayı engellemez; backend `OrderIndex` DTO'da) |
| SC-E21 | 16 §6.1, §9.1-M1/M2 | **Kaynak + açılış bloğu** (T6.19): her yayımlanmış ders **tam bir `Source` bloğuyla KAPANIR** ("Bu bilgiler nereden geliyor?") **ve** ilk bloğu öğrenme çıktılarını listeleyen açılış bloğudur (`Core/Explain`, ≥3 madde); kaynak bloğu **örnek sayıların kurgusal olduğunu beyan eder** ve **"yatırım tavsiyesi değildir"** çerçevesini taşır (`CLAUDE.md` §2); kaynak bloğu da tavsiye/tahmin taramasından geçer | Integration (seed) | [x] (2026-07-22; Education 16/16; **canlıda uçtan uca teyit** — mutabakat 5 derse 10 blok indirdi (44→54), Ders 1 adım 15/16 kaynak bloğu, `tuik.gov.tr` bağlantısı `rel="noopener noreferrer"` + `target="_blank"` ile canlıda doğrulandı) |
| SC-E22 | 16 §9.1-M3/M4/M5/M6 | **Müfredat–seed yapısal sözleşmesi:** figür eşiği set bazlı (Set 0 ≥6, diğer ≥2) ve **her figür anahtarı `LessonFigure.tsx` kayıt defterinde tanımlı** (sessizce düşen figür yok); ders başına 9 soru / her zorluktan ≥3 / tam 1 doğru + açıklama; her dersin ≥1 kavramı ve **boşta kavram yok** | Integration (seed) | [~] (2026-07-26 T6.20; **M4** figür↔kayıt defteri mutabakatı · **M6** boşta kavram yok + her ders ≥1 kavram · **M5** 9+ soruluk quiz her zorluktan ≥3 · Ders 1-2 zenginlik guard'ı **yeşil**. ⚠ **Set bazlı sayısal eşikler** (Set 0 ≥6/Set 2 ≥8/diğer ≥2; 9 soru/ders) içerik indikçe açılır — Ders 3-5 bugün 1 figür/3 soru, §9.3 gereği kırmızı bırakılmadı) |
| SC-E29 | 16 §6.4, `14` §B1, `CLAUDE.md` §3.1 | **Enflasyon verisi dürüstlüğü (T6.21):** seed'lenen `InflationRate` bir **placeholder** olduğundan kurumsal kaynakla (TÜİK/TCMB/SPK/BDDK) etiketlenemez — `Source = "örnek"`; türetilen `real_return` LiveContext çıktısı da örnek enflasyona dayandığını **beyan eder**. Gerçek TÜFE beslemesi bağlanınca kurum + veri tarihiyle güncellenir | Integration (seed) | [x] (2026-07-26; `Seeded_inflation_is_labeled_example_not_an_institution` + S1-L1 bağlam bloğu uyarısı; EducationSeed 21/21) |
| SC-E23 | 15 §6.2, `CLAUDE.md` §3.1 | **Enflasyon kaydırıcısı:** hesap **istemcide saf ve deterministik** (aynı girdi → aynı çıktı, sunucu/LLM yok); klavyeyle sürülebilir ve değer metin olarak da okunur; **varlık adı / gelecek tahmini cümlesi üretmez**; bileşen düşerse ders **statik figürle** çalışmaya devam eder | Bileşen | [ ] |
| SC-E24 | 16 §6.1, `11` §4, `14` §4-D2 | **MiniMarkdown tablo + bağlantı** (T6.8): `[metin](hedef)` bağlantıya çevrilir — dış adres `target="_blank" rel="noopener noreferrer"`, uygulama içi `/yol` ve `mailto:` aynı sekmede; **güvensiz hedef (javascript:/data:/vbscript:, protokole göreli `//host`, kontrol karakteriyle gizlenmiş şema) bağlantı ÜRETMEZ** — ham metin kalır; boru işaretli tablo **yalnız hizalama satırı varsa** tablodur (yoksa düz paragraf — geriye dönük), sütun sayısı başlıktan gelir (eksik hücre boş, fazlası düşer), hizalama `:-:`/`---:` okunur, tablo **kendi kabında** kaydırılır (sayfa yatay taşmaz); `dangerouslySetInnerHTML` **YİNE YOK** | Bileşen (13) + Integration (1) | [x] (2026-07-22; web 130/130, Integration 15/15; seed içerik kısıtı güncellendi: tablo/link serbest ama **şema denetimli** + hizalama satırı zorunlu) |
| SC-E25 | 16 §6.6, §9.1-M7a | **Grafik dili guard'ı** (Set 2): anlatım bloklarında (`Explain`/`Example`/`LiveContext`/`Source`) tahmin kalıbı **yok** — "formasyon ⇒ …", "sinyal verdi", "boşluk kapanır", "destek/direnç kırıldı ⇒ …", "hedef fiyat"; aynı kalıplar `Trap` bloğunda ve quiz **çeldiricisinde serbest** (orada yanlışın kendisi öğretiliyor) → test `SectionKind`'a bakarak ayırır | Integration (seed) | [ ] |
| SC-E26 | 16 §12-K3, §9.1-M7b | **Ölçüt beyaz listesi:** ders metninde geçebilen tek özel ad **geniş endeks** adıdır (beyaz liste); tek şirket · fon · aracı kurum adı **yok**; endeks adı geçen cümlede **eylem fiili** (al/sat/geç/kaydır) bulunmuyor | Integration (seed) | [ ] |
| SC-E27 | 15 §6.2, `CLAUDE.md` §3.1 | **Ölçek oynatıcısı** (S2-L2): seri **tohumlu ve saf** (aynı tohum → aynı grafik, sunucu/LLM yok); üç düğme (sıfır tabanlı eksen · log · dönem penceresi) **klavyeyle** erişilebilir ve durum metin olarak da okunur; çıktıda **yön yorumu / varlık adı yok**; bileşen düşerse ders **statik üç panelli figürle** çalışır | Bileşen | [ ] |
| SC-E28 | 16 §6.6-K3, `15` §3.4 | **Göreli performans sözleşmesi** (S2-L8): karşılaştırma çıktısı **dört ön koşulu** (aynı dönem · aynı para birimi · aynı ölçek · uygun ölçüt) beyan eder, **"geçmiş bir ölçümdür, gelecek göstergesi değildir"** ibaresini taşır ve **eylem cümlesi üretmez**; iki enstrümanı sıralayan form guard'a takılır; ölçüt serisi yokken **kurgusal/etiketli** örnek kullanılır (gerçekmiş gibi gösterilmez) | Integration + Bileşen | [ ] |
