...
1. **T0.4-T0.6** — EF Core + Npgsql + `FinansDbContext` + entity'ler (portföy +
   kimlik/audit iskeleti) + ilk migration + `T0.6b` kapsamlı seeder
2. **T0.9** — Tasarım token'ları (`DESIGN.md` → `@finans/shared/theme` TS+CSS) +
   web'de uygula + fontlar (Fraunces/Hanken)
3. **T0.12** — Serilog yapılandırılmış log + `/health` & `/health/ready`
4. **T0.13** — Güvenlik temeli (User Secrets/env + hata maskeleme + CORS allow-list)
5. **T0.14** — Docker (API Dockerfile non-root + compose: api + postgres)
6. **T0.11 kalan** — Sqlite integration fixture + Playwright iskeleti
