...
2. **T6.2** — "Senin portföyünde" bağlam API'si (`LessonContextService`, `ContextKey`,
   3 durum `Own`/`Demo`/`Stale`) — içerik hazır, kişiselleşme sırada
3. **T6.7** — Uyarlanabilir render (seviyeye göre L1/L2/L3 + "Daha derine in")
4. **T6.6** — Tanılama testi (8 soru; eski T7.1'i devraldı) → `LiteracyLevel`/`RiskAttitude`
5. OSS kalanı — README ekran görüntüleri tazeleme (pano + Eğitim artık canlı)
