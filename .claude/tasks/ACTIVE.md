...
1. **T0.12** — Serilog yapılandırılmış log + `/health` & `/health/ready`
2. **T0.13** — Güvenlik temeli (User Secrets/env + hata maskeleme + CORS allow-list)
3. **T0.14** — Docker (API Dockerfile non-root + compose: api + postgres)
4. **T0.11 kalan** — Sqlite integration fixture + Playwright iskeleti
...
- **T0.4-T0.6b**: EF Core veri katmanı + 12 tablo + migration + tutarlı seed
  (422.970/641.403/+%51,6) — `main`'e merge edildi
- **T0.9**: tasarım token'ları + fontlar (DESIGN.md → web, görsel doğrulandı) —
  dal `feat/design-tokens`
