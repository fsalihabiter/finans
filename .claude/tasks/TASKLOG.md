...
- **Ne yapıldı:** Ürün sahibi tespiti: eğitim *"yatırım nedir"den başlayıp
  **grafik okuma · mum grafiği okuma · piyasaya göre kalem yorumlama**ya kadar*
  gitmeli. Mevcut 25 derslik müfredat bu halkayı **hiç** içermiyordu — fiyatın
  nasıl oluştuğu, borsanın/aracı kurumun ne olduğu, bir grafiğin eksenleri, mum
  grafiği, endeks ve göreli performans **hiçbir derste geçmiyordu**. Kullanıcı,
  portföyünü okuyabilen ama ekranındaki grafiği okuyamayan bir noktada
  bırakılıyordu.
  (1) **Yeni Set 2 — "Grafik ve Piyasa Okuryazarlığı" (8 ders)**, künyeleri tam
  detayda yazıldı (`16` §5): grafik eksenleri → **ölçek ve dönem** (ölçek
  yalanı) → **mum anatomisi (OHLC)** → zaman dilimi/boşluk → hacim ve emir
  defteri → **"grafik geleceği söylemez"** → endeks ve ölçüt → **kalemini
  piyasaya göre okumak**. Her künye: 14-15 aşama · 8-11 figür (≥3 çok panelli) ·
  9 soru/3 zorluk · kaynak · ⚠ yasal not.
  (2) **Set 0'a iki ders** (S0-L9 *Fiyat nereden çıkıyor?* · S0-L10 *Bir alım
  nasıl gerçekleşir?*) — grafik okumanın ön koşulu: grafikteki nokta **birinin
  birine bir şey sattığı andır**.
  (3) **Kavram haritası 64 → 103**; `period-selection` tanıtımı S3-L4'ten
  **S2-L2'ye taşındı** (§2.3 spirali: sezgi → formül → kendi verisi).
  (4) **Setler yeniden numaralandı:** eski Set 2/3/4 → **3/4/5** (kod etkisi
  yok — o setlerin içeriği henüz yazılmamıştı, T6.11/12/13 açık).
  (5) `16` §6.6 **grafik ve karşılaştırma dili** kuralı + §8.3'e **grafik SVG
  öğeleri** (`ChartFrame` ölçek parametreli · `Candle` · `VolumeBars` ·
  `BookLadder` …) + §9.1'e **M7a/M7b** + §9.2'ye **İ6** eklendi.
- **Dokunulan dosyalar:** `.claude/docs/16-CURRICULUM.md` (+~1000 satır: §3 harita,
  §5 10 yeni künye, §6.6, §8.3, §9, **§12 kararlar**), `.claude/docs/15-EDUCATION-PLAN.md`
  (§6 set yapısı 6 set/35 ders, §6.1 çıta tablosu, §6.2 ikinci araç, §7 senaryolar,
  §8 görev kırılımı), `.claude/docs/08-BACKLOG.md` (T6.22-T6.27 + DoD 25→35),
  `.claude/docs/09-TESTING-STRATEGY.md` (SC-E25-E28), `.claude/tasks/ACTIVE.md`.
- **Test:** yok — **kod değişmedi**, yalnız müfredat/planlama dokümanları.
  Yeni senaryolar katalogda **açık** olarak duruyor: **SC-E25** grafik dili
  guard'ı (M7a) · **SC-E26** ölçüt beyaz listesi (M7b) · **SC-E27** ölçek
  oynatıcısı · **SC-E28** göreli performans sözleşmesi. Testler `16` §9.3 gereği
  **içerikten sonra** yazılır (T6.27), kırmızı test bırakılmaz.
- **Karar/Not:** (`16` §12 K1-K7)
  **K2 — sıra:** yeni set Set 1'den sonra, "Portföyünü Okumak"tan **önce**;
  gerekçe: Nirengi'nin kendi **Değer Seyri** grafiğini okumak portföy
  derslerinin ön koşulu.
  ⚖ **K3 — onay bekleyen:** **geniş endeks adları izinli** (BIST 100, S&P 500
  gibi — bunlar **ölçüt**tür, alınıp satılması önerilen enstrüman değil), tek
  şirket · fon · aracı kurum adı **yasak** kalır (`15` §3.4). "Piyasaya göre"
  öğretmenin başka yolu yok; ama bu bir **hukuki sınır kararıdır** → lansman
  öncesi SPK/fintech incelemesinde ayrıca teyit edilecek (`14` §6).
  **K4 — teknik analiz:** yok sayılmıyor, **bir ders konusu** yapılıyor (S2-L6).
  İki yönlü sınır: ders **sinyal üretmez**, teknik analizin "işe yaramadığını"
  da **iddia etmez** (o da kaynaklandırılamayan bir hüküm). Öğretilen şey
  **iddiayı sınama becerisi** (örneklem · dönem · maliyet · yanlışlanabilirlik).
  **K6:** setin tek etkileşimli aracı **ölçek oynatıcısı** (`15` §6.2 "set
  başına 1" korunuyor).
  **K7:** mum içeriği **veri gerektirmez** (figürler kurgusal); yalnız S2-L8'in
  canlı ölçüt karşılaştırması yeni veriye bağlı (T6.26) — 8 dersin 7'si bugünkü
  altyapıyla yazılabilir. Uygulamada mum grafiği **göstermek** ayrı bir ürün
  işidir, müfredatın ön koşulu değil.
  ⚠ **Kod etkisi yok:** aynı `LessonSection` şeması, aynı `DepthTier`/
  `SectionKind` enum'ları, **migration gerekmez**.
  ⚠ **İş yükü dürüstlüğü:** Set 0 (2 yeni ders dahil) ~80 figür + 90 soru,
  Set 2 ~75 figür + 72 soru. Bunu mümkün kılan şey **paylaşılan SVG öğeleridir**
  (T6.23): "aynı veri, üç görüntü" figürleri **aynı seriyi** farklı ölçek
  parametreleriyle çizerek kurulur, üç ayrı figür çizilmez.
- **Durum:** tamamlandı (planlama turu).
- **Sıradaki:** sıra değişmedi — **T6.20** (yapısal sözleşme testleri) →
  **T6.21** (enflasyon verisi) → **T6.15** (çok set desteği; artık **6 set**
  listeleyecek) → T6.16/T6.17 (Set 0) → **T6.22/T6.23** (Set 2 iskeleti +
  grafik SVG öğeleri).

## 2026-07-22 · T6.19 — Kaynak bloğu + açılış bloğu (tüm dersler)
- **Görev(ler):** T6.19 (`16` §2.5 ders iskeleti · §6.1 kaynak politikası).
- **Ne yapıldı:** (1) **Açılış bloğu** — her ders artık **"Bu derste ne
  öğreneceksin?"** ile başlıyor: öğrenme çıktıları madde madde (`16` §5
  künyelerinden). Yeni `SectionKind` gerekmedi; konum + başlık yeterli, böylece
  şema/migration dokunulmadı. Aşamalı okuyucunun 1. adımı ve yol haritasının ilk
  başlığı oldu. (2) **Kaynak bloğu** — her ders **"Bu bilgiler nereden geliyor?"**
  ile kapanıyor. **Kullanılmayan `SectionKind.Source` nihayet devrede**
  (`Enums.cs:188`, T6.5'ten beri boştu). Blok **dördü birden** söylüyor:
  kurumsal kaynak (TÜİK · KAP · SPK — T6.8 bağlantılarıyla tıklanabilir),
  **örnek sayıların kurgusal olduğu beyanı**, hesapların **kodda** yapıldığı ve
  LLM'in sayı üretmediği (`CLAUDE.md` §3.1), **"yatırım tavsiyesi değildir"**
  çerçevesi (`CLAUDE.md` §2 — artık her derste görünür).
  (3) `Intro`/`Src` yardımcıları + `Blocks` kalıbı iki parametreyle genişletildi.
- **Dokunulan dosyalar:** `Finans.Infrastructure/Seed/EducationContent.cs`,
  `tests/Finans.Integration.Tests/EducationSeedTests.cs`, `web/src/App.css`,
  docs (08, 09), ACTIVE.
- **Test:** **SC-E21** — yeni `Every_lesson_opens_with_outcomes_and_closes_with_sources`:
  ilk blok açılış (`Core/Explain`, ≥3 çıktı) · **tam bir** `Source` bloğu ve
  **son** aşama · "kurgusal" beyanı · "yatırım tavsiyesi değildir" çerçevesi.
  Education **16/16**, Application **291/291**, Integration **168/172** (aynı 4
  bilinen), web **130/130**. **Canlı teyit:** `docker compose up -d --build api`
  → mutabakat 5 derse 10 blok indirdi (**44→54**); Ders 1 canlıda 15 bölüm,
  adım 15/16 kaynak bloğu; `tuik.gov.tr` bağlantısı tarayıcıda
  `target="_blank"` + `rel="noopener noreferrer"` ile doğrulandı.
- **Karar/Not:** **Kaynak uydurulmadı** — kurum adı ve ne yayımladığı yazıldı,
  **belge adı/tarihi verilmedi**; bağlantılar yalnız kurum ana sayfalarına
  (deep link doğrulanmadan yazılmaz, `16` İ2). Enflasyon cümlesi bilinçli olarak
  *"uygulamadaki oran TÜİK'ten gelir"* **demiyor** — çünkü gelmiyor (bkz. T6.21).
  ⚠ **Yakalanan iki hata:** (a) Ders 2'de açılış bloğu ilk turda **atlandı** —
  sabit bölüm sayısı testi (44) yakaladı, yeni yapısal test tek başına da
  yakalardı; (b) **canlıda görüldü:** kaynak bloğunun görsel ayrımı yoktu
  (tuzak/örnek/senin-verin blokları aksan çizgisi taşıyor, kaynak gövde metniyle
  aynı görünüyordu) → `kind-source` **kesikli çizgi + sönük ton** aldı, künye
  gibi okunuyor. ⚠ **Tarayıcı erişimi:** `https://localhost` Chrome'da sertifika
  interstitial'ı veriyor ve eklenti o sayfaya bağlanamıyor; çözüm **Vite dev
  sunucusunu `VITE_API_TARGET=https://localhost` ile HTTP üzerinden** koşmak —
  compose verisi görülüyor, sertifika sorunu yok. Sonraki oturumlar için yöntem bu.
  🔴 **Yeni bulgu → T6.21:** `SeedData` `InflationRate`'i **`Source="TÜİK"`
  etiketiyle sabit %38** yazıyor (kod yorumu "placeholder" diyor). Eğitim artık
  "kaynak daima görünür" iddiası taşıdığından, TÜİK etiketli uydurma bir oran bu
  iddiayı çürütür — backlog'a alındı.
- **Durum:** tamamlandı.
- **Sıradaki:** **T6.20** — yapısal sözleşme testleri (`16` §9.1 M3-M9; M1/M2
  bu görevle yeşil), ardından **T6.21** enflasyon verisi.

## 2026-07-22 · T6.8 — `MiniMarkdown`: bağlantı + tablo (şema beyaz-listeli)
- **Görev(ler):** T6.8 (müfredat sırasında **öne alındı** — `Source` bloğundaki
  kurumsal referansların tıklanabilir olmasının önkoşulu, `16` §6.1).
- **Ne yapıldı:** (1) **Bağlantı** `[metin](hedef)`: hedef **şema beyaz-listesinden**
  geçer (`safeHref`) — `https:`/`http:`/`mailto:` ve uygulama içi `/yol`. Güvensiz
  hedef **bağlantıya çevrilmez**, kaynak metin okunur biçimde kalır (içerik
  kaybolmaz, tıklanabilir saldırı yüzeyi oluşmaz). Engellenenler: `javascript:` ·
  `data:` · `vbscript:` · **protokole göreli `//host`** (dışarı çıkar ama iç
  bağlantı gibi görünür) · **kontrol karakteriyle gizlenmiş şema**
  (`java<TAB>script:` tarayıcıda çözülür). Dış bağlantı `target="_blank"` +
  **`rel="noopener noreferrer"`** + `aria-hidden` ok işareti; `mailto:` ve iç yol
  aynı sekmede. (2) **Tablo**: boru işaretli GFM alt kümesi. Tablo **yalnızca
  başlık satırını hizalama satırı izliyorsa** başlar — tek başına `|` içeren satır
  eskisi gibi düz paragraf kalır (geriye dönük davranış korundu). Sütun sayısı
  **başlıktan** gelir: eksik hücre boş, fazlası düşer; `:-:`/`---:` hizalaması
  okunur (TR para sütunu sağa); tablo **kendi kabında** (`.md-table-wrap`) yatay
  kaydırılır — dar ekranda **sayfa taşmaz**.
- **Dokunulan dosyalar:** `web/src/components/{MiniMarkdown.tsx,MiniMarkdown.test.tsx}`,
  `web/src/App.css`, `backend/tests/Finans.Integration.Tests/EducationSeedTests.cs`,
  `backend/src/Finans.Infrastructure/Seed/EducationContent.cs` (içerik kuralı
  docblock'u), docs (08, 09).
- **Test:** **SC-E24** (`09` §5). web **130/130** (13 yeni bileşen testi: dış/iç/
  mailto bağlantı · 4 güvensiz şema · gizlenmiş şema · bağlantı metnindeki HTML ·
  tablo çizimi + hizalama · hücrede kalın/bağlantı · düzensiz satır · hizalama
  satırsız `|`), `npm run build` temiz. Backend Application **291/291**,
  Integration **167/171** (aynı 4 bilinen `WebApplicationFactory` hatası — sayı
  değişmedi). Seed içerik kısıtı **gevşetilmedi, dönüştürüldü**: `|`/`](` yasağı
  kalktı ama artık **her bağlantı hedefi şema denetiminden** geçiyor ve **tablo
  hizalama satırı zorunlu** (yoksa içerik ekranda sessizce paragrafa düşerdi).
- **Karar/Not:** Güvensiz hedefte **bağlantıyı düşürüp ham metni bırakmak**
  bilinçli seçim — alternatif (bloğu tamamen atmak) içeriği kaybettirir, `href`'i
  boşaltmak ise tıklanabilir ama ölü bir öğe bırakır. ⚠ **Yaşanan tuzak:** şema
  gizleme temizliği için yazılan `[ - ]` regex'i kaynak dosyaya **gerçek
  kontrol karakteri** olarak indi → karakter **koduyla** süzen `stripHiddenChars`
  fonksiyonuna geçildi (kaynakta kontrol karakteri taşınmıyor). ⚠ **Tarayıcı
  doğrulaması YAPILAMADI:** Chrome sekmesi `https://localhost` ve `http://127.0.0.1`
  için hata sayfası gösterdi (curl 200 dönüyor, compose sağlıklı). Görsel teyit
  **T6.19'a** ertelendi — zaten mevcut derslerde tablo/bağlantı içeriği yok, ilk
  gerçek kullanım kaynak bloklarıyla gelecek.
- **Durum:** tamamlandı.
- **Sıradaki:** **T6.19** — `Source` bloğu + "Bu derste ne öğreneceksin?" açılış
  bloğu (mevcut 5 derse geriye dönük); ardından **T6.20** sözleşme testleri.
